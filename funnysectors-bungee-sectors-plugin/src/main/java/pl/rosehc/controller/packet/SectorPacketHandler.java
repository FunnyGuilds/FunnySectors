...
import pl.rosehc.sectors.SectorsConfiguration.SectorWrapper;
...
public final class SectorPacketHandler implements PacketHandler,
  ...
  public void handle(final ConfigurationSynchronizePacket packet) {
    if (packet.getConfigurationName()
        .equals("pl.rosehc.controller.configuration.impl.SectorsConfiguration")
      ...
      this.plugin.getProxyFactory().updateCurrentProxy();
  ...
  public void handle(final SectorUserConnectPacket packet) {
    if (this.plugin.isLoaded()) {
      ProxiedPlayer player = ProxyServer.getInstance().getPlayer(packet.getUniqueId());
      if (Objects.nonNull(player)) {
        this.plugin.getSectorFactory().findSector(packet.getSectorName())
            .flatMap(Sector::getServerInfo).ifPresent(player::connect);
      }
    }
  }
  ...
  public void handle(final SectorUserCreatePacket packet) {
    if (this.plugin.isLoaded()) {
      this.plugin.getSectorFactory().findSector(packet.getSectorName()).ifPresent(
          sector -> this.plugin.getProxyFactory().findProxy(packet.getProxyIdentifier()).ifPresent(
              proxy -> this.plugin.getSectorUserFactory().addUser(
